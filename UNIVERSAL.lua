-- WindUI Universal Script v3.2
-- Fixed toggles with Default = false and re-added ESP Highlight

-- Load WindUI with error handling and fallback
local WindUI
local success, err = pcall(function()
    WindUI = loadstring(game:HttpGet("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua", true, 10))()
end)

if not success then
    warn("Failed to load WindUI: " .. tostring(err))
    success, err = pcall(function()
        WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/Footagesus/WindUI/main/main.lua", true, 10))()
    end)
    if not success then
        error("Critical: Unable to load WindUI library. Script cannot proceed.")
    end
end
...
-- Simplified Confirmation Popup
local Confirmed = false
WindUI:Popup({
    Title = gradient("SNT&MIRROZZ HUB v3.2", Color3.fromHex("#eb1010"), Color3.fromHex("#1023eb")),
    Icon = "info",
...
-- Timeout for popup to prevent freeze
local startTime = tick()
while not Confirmed and tick() - startTime < 10 do
    task.wait()
end

if not Confirmed then
    Notify("Error", "Script load timed out. Retrying...", "x-circle")
    return
end

Notify("SNT&MIRROZZ HUB", "Script successfully loaded!", "check-circle")

-- Create Window
local Window = WindUI:CreateWindow({
    Title = gradient("SNT&MIRROZZ HUB", Color3.fromHex("#001e80"), Color3.fromHex("#16f2d9")),
    Icon = "infinity",
    Author = gradient("Universal | Version: 3.2", Color3.fromHex("#1bf2b2"), Color3.fromHex("#1bcbf2")),
...
local Tabs = {
    ...
    WindowTab = Window:Tab({ Title = gradient("CONFIGURATION", Color3.fromHex("#ffffff"), Color3.fromHex("#636363")), Icon = "settings" }),
    CreateThemeTab = Window:Tab({ Title = gradient("THEMES", Color3.fromHex("#ffffff"), Color3.fromHex("#636363")), Icon = "palette" }),
}
...
local function GetLocalTimeOfDay()
    ...
    local timeOfDay = (hour >= 6 and hour < 12 and period == "AM" and "Morning") or
                      (hour >= 12 and hour < 6 and period == "PM" and "Afternoon") or
                      (hour >= 6 and hour < 10 and period == "PM" and "Evening") or "Night"
...
local fpsParagraph = Tabs.MainTab:Paragraph({ Title = gradient("FPS", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")), Desc = "FPS: Calculating..." })
local pingParagraph = Tabs.MainTab:Paragraph({ Title = gradient("Ping", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")), Desc = "Ping: Calculating..." })
local injectTimeParagraph = Tabs.MainTab:Paragraph({ Title = gradient("Time Since Inject", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")), Desc = "Time: 00:00:00" })
local localTimeParagraph = Tabs.MainTab:Paragraph({ Title = gradient("Local Time", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")), Desc = "Time: Calculating..." })
local currentPlayersParagraph = Tabs.MainTab:Paragraph({ Title = gradient("Current Players", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")), Desc = "Players: Calculating..." })
local maxPlayersParagraph = Tabs.MainTab:Paragraph({ Title = gradient("Max Players", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")), Desc = string.format("Max Players: %d", PlayerInfo.MaxPlayers) })
...
RunService.Heartbeat:Connect(function()
    ...
    PlayerInfo.Ping = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()
...
local function ToggleFly(state)
    ...
    if state then
        ...
        Notify("Fly", "Fly enabled", "check-circle")
    else
        ...
        Notify("Fly", "Fly disabled", "x-circle")
    end
end
...
local function UpdateESP()
    for _, player in pairs(Players:GetPlayers()) do
        ...
        if ESPSettings.Highlights then
            ...
            objects.Highlight.FillColor = ESPSettings.HighlightFillColor
            objects.Highlight.OutlineColor = ESPSettings.HighlightOutlineColor
            objects.Highlight.FillTransparency = ESPSettings.HighlightFillTransparency
...
Tabs.EspTab:Toggle({
    Title = "Distance ESP",
    Default = false,
    Callback = function(state) ESPSettings.Distance = state end
})
Tabs.EspTab:Toggle({
    Title = "Health ESP",
    Default = false,
    Callback = function(state) ESPSettings.Health = state end
})
Tabs.EspTab:Toggle({
    Title = "Tracers",
    Default = false,
    Callback = function(state) ESPSettings.Tracers = state end
})
Tabs.EspTab:Toggle({
    Title = "Boxes",
    Default = false,
...
Tabs.EspTab:Toggle({
    Title = "Highlights",
    Default = false,
    Callback = function(state) ESPSettings.Highlights = state end
})
...
Tabs.EspTab:Colorpicker({
    ...
    Callback = function(color) ESPSettings.TextColor = color end
})
Tabs.EspTab:Slider({
    Title = "Text Size",
    Value = { Min = 12, Max = 24, Default = 16 },
    Callback = function(value) ESPSettings.TextSize = value end
})
Tabs.EspTab:Slider({
    Title = "Transparency",
    Value = { Min = 0, Max = 1, Default = 0.5 },
    Callback = function(value) ESPSettings.Transparency = value end
})
Tabs.EspTab:Dropdown({
    Title = "Tracer Origin",
    Values = {"Bottom", "Center"},
    Value = "Bottom",
    Callback = function(value) ESPSettings.TracerOrigin = value end
})
Tabs.EspTab:Colorpicker({
    Title = "Highlight Fill Color",
    Default = ESPSettings.HighlightFillColor,
    Callback = function(color) ESPSettings.HighlightFillColor = color end
})
Tabs.EspTab:Colorpicker({
    Title = "Highlight Outline Color",
    Default = ESPSettings.HighlightOutlineColor,
    Callback = function(color) ESPSettings.HighlightOutlineColor = color end
})
Tabs.EspTab:Slider({
...
Tabs.TeleportTab:Input({
    Title = "X Coordinate",
    Placeholder = "X",
    Callback = function(value) TeleportSettings.Coordinates = Vector3.new(tonumber(value) or 0, TeleportSettings.Coordinates.Y, TeleportSettings.Coordinates.Z) end
})
Tabs.TeleportTab:Input({
    Title = "Y Coordinate",
    Placeholder = "Y",
    Callback = function(value) TeleportSettings.Coordinates = Vector3.new(TeleportSettings.Coordinates.X, tonumber(value) or 0, TeleportSettings.Coordinates.Z) end
})
Tabs.TeleportTab:Input({
    Title = "Z Coordinate",
    Placeholder = "Z",
    Callback = function(value) TeleportSettings.Coordinates = Vector3.new(TeleportSettings.Coordinates.X, TeleportSettings.Coordinates.Y, tonumber(value) or 0) end
})
Tabs.TeleportTab:Button({
...
Tabs.TeleportTab:Section({ Title = gradient("Saved Positions", Color3.fromHex("#00b7eb"), Color3.fromHex("#003087")) })
local positionName = ""
Tabs.TeleportTab:Input({
    Title = "Position Name",
    Placeholder = "Enter position name",
    Callback = function(name) positionName = name end
})
Tabs.TeleportTab:Button({
...
local positionDropdown = Tabs.TeleportTab:Dropdown({
    Title = "Saved Positions",
    Values = table.keys(TeleportSettings.SavedPositions),
...
Players.PlayerAdded:Connect(function() teleportDropdown:Set("Values", UpdateTeleportPlayers()) end)
Players.PlayerRemoving:Connect(function() teleportDropdown:Set("Values", UpdateTeleportPlayers()) end)
...
-- Auto Execute
local AutoInjectSettings = {
    Enabled = false,
    ScriptURL = "https://raw.githubusercontent.com/Snowt-Team/SNT-HUB/refs/heads/main/UNIVERSAL.lua",
    RetryAttempts = 3,
    RetryDelay = 5,
}

-- Load saved Auto Execute settings
local autoInjectData = LoadFile("AutoInject")
if autoInjectData then
    AutoInjectSettings.Enabled = autoInjectData.Enabled or false
    AutoInjectSettings.ScriptURL = autoInjectData.ScriptURL or AutoInjectSettings.ScriptURL
end

local function ExecuteScript(url)
    local attempts = 0
    while attempts < AutoInjectSettings.RetryAttempts do
        local success, result = pcall(function()
            return loadstring(game:HttpGet(url, true, 10))()
        end)
        if success then
            Notify("Auto Inject", "Script executed successfully", "check-circle")
            return true
        else
            attempts = attempts + 1
            Notify("Auto Inject", "Attempt " .. attempts .. " failed: " .. tostring(result), "x-circle")
            task.wait(AutoInjectSettings.RetryDelay)
        end
    end
    Notify("Auto Inject", "Failed to execute script after " .. AutoInjectSettings.RetryAttempts .. " attempts", "x-circle")
    return false
end
...
local function SetupAutoInject()
    ...
    SaveFile("AutoInject", {
        Enabled = AutoInjectSettings.Enabled,
        ScriptURL = AutoInjectSettings.ScriptURL
    })

    spawn(function()
        task.wait(2)
        ExecuteScript(AutoInjectSettings.ScriptURL)
    end)

    LocalPlayer.OnTeleport:Connect(function(state)
        if state == Enum.TeleportState.Started and AutoInjectSettings.Enabled then
            queue_on_teleport([[
                local attempts = 0
                local maxAttempts = ]] .. AutoInjectSettings.RetryAttempts .. [[
                local delay = ]] .. AutoInjectSettings.RetryDelay .. [[
                while attempts < maxAttempts do
                    local success, err = pcall(function()
                        loadstring(game:HttpGet("]] .. AutoInjectSettings.ScriptURL .. [[", true, 10))()
                    end)
                    if success then
                        break
                    end
    ...
    Players.PlayerRemoving:Connect(function(player)
        if player == LocalPlayer and AutoInjectSettings.Enabled then
            queue_on_teleport([[
                local attempts = 0
                local maxAttempts = ]] .. AutoInjectSettings.RetryAttempts .. [[
                local delay = ]] .. AutoInjectSettings.RetryDelay .. [[
...
Tabs.SettingsTab:Input({
    Title = "Script URL",
    Value = AutoInjectSettings.ScriptURL,
    Placeholder = "Enter script URL...",
    Callback = function(url)
        AutoInjectSettings.ScriptURL = url
        SaveFile("AutoInject", {
            Enabled = AutoInjectSettings.Enabled,
            ScriptURL = AutoInjectSettings.ScriptURL
        })
...
Tabs.SettingsTab:Button({
    Title = "Manual Inject",
    Callback = function()
        if ExecuteScript(AutoInjectSettings.ScriptURL) then
            Notify("Auto Inject", "Manual inject executed", "check-circle")
        end
    end
})
Tabs.SettingsTab:Slider({
    ...
    Callback = function(value) AutoInjectSettings.RetryAttempts = value end
})
Tabs.SettingsTab:Slider({
    Title = "Retry Delay (s)",
    Value = { Min = 1, Max = 10, Default = 5 },
    Callback = function(value) AutoInjectSettings.RetryDelay = value end
})

-- Settings Tab (Other Features)
local Settings = {
...
local function SaveConfig()
    ...
    SaveFile(Settings.Config.CurrentConfigName, configData)
...
local function LoadConfig(configName)
    ...
    if data then
        ...
        CharacterSettings = data.CharacterSettings or CharacterSettings
        Settings = data.Settings or Settings
        Notify("Config", "Config '" .. configName .. "' loaded.", "check-circle")
        return true
    else
        Notify("Config", "Failed to load config '" .. configName .. "'.", "x-circle")
        return false
    end
end
...
Tabs.SettingsTab:Section({ Title = gradient("Character Modifications", Color3.fromHex("#ffffff"), Color3.fromHex("#636363")) })
Tabs.SettingsTab:Toggle({
    Title = "Noclip",
    Default = false,
        ...
        if SetDebounce("LowServer", Config.DebounceTime) then
            ...
            pcall(function()
                ...
                if #validServers > 0 then
                    ...
                    TeleportService:Teleport(placeId)
                    Notify("Server", "No servers found, joining new server...", "server")
                end
            end)
        end
    end
...
Tabs.SocialsTab:Paragraph({
    ...
    Color = "Green",
    Buttons = {
        { Title = "TG Channel", Icon = "link", Callback = function() setClipboard("t.me/supreme_scripts") end }
    }
})

Tabs.SocialsTab:Paragraph({
    Title = gradient("Mirrozz", Color3.fromHex("#ffffff"), Color3.fromHex("#636363")),
    Desc = "My Friend's Socials",
    Image = "star",
    Color = "Green",
    Buttons = {
        { Title = "TG Channel", Icon = "link", Callback = function() setClipboard("t.me/mirrozzscript") end }
    }
})

-- Changelogs Tab
Tabs.ChangelogsTab:Section({ Title = gradient("Changelogs", Color3.fromHex("#ffffff"), Color3.fromHex("#636363")) })
Tabs.ChangelogsTab:Code({
    Title = "Changelogs v3.2:",
    Text = [[
        - Fixed missing Default = false in toggles for WindUI consistency
        - Re-added Highlight to ESP with configurable fill and outline colors
        - Maintained improved Auto Execute with retry logic
        - Enhanced injection reliability with fallback URL
        - Updated socials with correct icons and clipboard functionality
    ]]
})

Tabs.ChangelogsTab:Code({
    Title = "Next Update (v3.3):",
    Text = [[
        - Team-based ESP filtering
        - Aimbot prediction for moving targets
        - Auto-farm for popular games
        - Additional troll features (chat spam, visual effects)
...
if AutoInjectSettings.Enabled then
    SetupAutoInject()
end
