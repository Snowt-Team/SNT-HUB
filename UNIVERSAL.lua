...
local function updateAimbot()
    if not AimbotConfig.Enabled then
        if AimbotData.Connections.MouseConnection then
            ...
            AimbotData.Connections.MouseConnection = nil
        end
        return
    end
    ...
    if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") and target.Character:FindFirstChild("Humanoid") and target.Character.Humanoid.Health > 0 then
        ...
        elseif Camera.CameraSubject ~= LocalPlayer.Character.Humanoid then
            Camera.CameraSubject = LocalPlayer.Character.Humanoid
        end

        if AimbotConfig.LockCamera and UserInputService.MouseBehavior == Enum.MouseBehavior.Default then
        ...
        if AimbotConfig.Spectate and Camera.CameraSubject ~= LocalPlayer.Character.Humanoid then
            Camera.CameraSubject = LocalPlayer.Character.Humanoid
        end
    end
end

-- Mouse hold detection for aimbot activation
...
Tabs.AimbotTab:Toggle({
    ...
    Callback = function(state)
        ...
        if not state and Camera.CameraSubject ~= LocalPlayer.Character.Humanoid then
            Camera.CameraSubject = LocalPlayer.Character.Humanoid
        end
    end
})
...
-- Initialize
local function Ainit()
    AimbotData.Connections.RenderStepped = RunService.RenderStepped:Connect(updateAimbot)
    AimbotData.Connections.PlayerRemoving = Players.PlayerRemoving:Connect(onPlayerRemoving)
    setupMouseHold()
    updateFOVCircle()
end

-- Cleanup
local function Acleanup()
