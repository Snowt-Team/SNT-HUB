...
local ESPSettings = {
    ...
    Tracers = {Enabled = false, Transparency = 0.5, Thickness = 0.05, Color = Color3.new(1, 0, 0), Type = "3D"},
...
local function CreateESP(player)
    ...
    -- Tracer Beam setup
    esp.TracerBeam.Parent = Workspace
    esp.TracerBeam.Width0 = ESPSettings.Tracers.Thickness
    esp.TracerBeam.Width1 = ESPSettings.Tracers.Thickness
    esp.TracerBeam.Enabled = false
    esp.Attachments.TracerStart = Instance.new("Attachment", Workspace.Terrain)
    esp.Attachments.TracerEnd = Instance.new("Attachment")
    esp.TracerBeam.Attachment0 = esp.Attachments.TracerStart
    esp.TracerBeam.Attachment1 = esp.Attachments.TracerEnd

    -- Tracer GUI setup (for 2D)
    esp.TracerGui.Adornee = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    esp.TracerGui.Parent = Workspace
    esp.TracerGui.Size = UDim2.new(0, 2, 0, 1000)
    esp.TracerGui.StudsOffset = Vector3.new(0, 0, 0)
    esp.TracerGui.AlwaysOnTop = true
    esp.TracerGui.Enabled = false
    esp.TracerFrame.Parent = esp.TracerGui
    esp.TracerFrame.Size = UDim2.new(1, 0, 1, 0)
    esp.TracerFrame.BackgroundTransparency = ESPSettings.Tracers.Transparency
    esp.TracerFrame.BackgroundColor3 = ESPSettings.Tracers.Color
...
local function UpdateESP()
    for player, esp in pairs(ESPObjects) do
        ...
        -- Check if player is valid and within distance
        local isValid = character and rootPart and head
        local distance = isValid and (rootPart.Position - (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character.HumanoidRootPart.Position or Camera.CFrame.Position)).Magnitude or math.huge
        local withinDistance = distance <= ESPSettings.ShowDistance

        -- Disable all ESP elements if player is invalid or out of range
        if not isValid or not withinDistance then
            esp.Highlight.Enabled = false
            esp.DistanceGui.Enabled = false
            esp.NameGui.Enabled = false
            esp.TracerBeam.Enabled = false
        ...
        esp.TracerGui.Adornee = rootPart
        ...
        -- Tracers
        if ESPSettings.Tracers.Type == "3D" then
            esp.TracerBeam.Enabled = ESPSettings.Tracers.Enabled
            esp.TracerGui.Enabled = false
            if esp.TracerBeam.Enabled then
                ...
                esp.TracerBeam.Color = ColorSequence.new(ESPSettings.Tracers.Color)
                esp.TracerBeam.Transparency = NumberSequence.new(ESPSettings.Tracers.Transparency)
                esp.TracerBeam.Width0 = ESPSettings.Tracers.Thickness
                esp.TracerBeam.Width1 = ESPSettings.Tracers.Thickness
            end
        else -- 2D
            esp.TracerBeam.Enabled = false
            esp.TracerGui.Enabled = ESPSettings.Tracers.Enabled
            if esp.TracerGui.Enabled then
                local rootPos, onScreen = Camera:WorldToViewportPoint(rootPart.Position)
                if onScreen then
                    local screenHeight = Camera.ViewportSize.Y
                    esp.TracerGui.Size = UDim2.new(0, ESPSettings.Tracers.Thickness * 2, 0, screenHeight - rootPos.Y)
                    esp.TracerGui.StudsOffset = Vector3.new(0, 0, 0)
                    esp.TracerFrame.BackgroundTransparency = ESPSettings.Tracers.Transparency
                    esp.TracerFrame.BackgroundColor3 = ESPSettings.Tracers.Color
                else
        ...
        -- Center the box on HumanoidRootPart
        local hrpPos = rootPart.Position
        local boxHeight = 4 * boxScale -- Total height of the box
        local boxWidth = 2 * boxScale -- Total width of the box
        esp.Attachments.BoxTopLeft.WorldPosition = hrpPos + Vector3.new(-boxWidth/2, boxHeight/2, 0)
        esp.Attachments.BoxTopRight.WorldPosition = hrpPos + Vector3.new(boxWidth/2, boxHeight/2, 0)
        esp.Attachments.BoxBottomLeft.WorldPosition = hrpPos + Vector3.new(-boxWidth/2, -boxHeight/2, 0)
        esp.Attachments.BoxBottomRight.WorldPosition = hrpPos + Vector3.new(boxWidth/2, -boxHeight/2, 0)
        for _, beam in pairs(esp.Box) do
...
Tabs.EspTab:Dropdown({
    ...
    Value = "3D",
    Callback = function(value)
        ESPSettings.Tracers.Type = value
    end
})
...
Tabs.EspTab:Slider({
    ...
    Callback = function(value)
        ...
        -- Force an immediate update to ensure ESP visuals reflect new distance
        UpdateESP()
    end
})