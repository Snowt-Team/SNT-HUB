...
local function RemoveESP(player)
    if ESPObjects[player] then
        ...
        if esp.TracerBeam then esp.TracerBeam:Destroy() end
        if esp.TracerGui then esp.TracerGui:Destroy() end
        for _, attachment in pairs(esp.Attachments) do
...
local function UpdateESP()
    ...
    for player, esp in pairs(ESPObjects) do
        ...
        local isValid = character and rootPart and head and player ~= LocalPlayer
        local distance = isValid and (rootPart.Position - localPos).Magnitude or math.huge
        local withinDistance = isValid and distance <= ESPSettings.ShowDistance
...
local espConnection = RunService.Heartbeat:Connect(function()
    if next(ESPObjects) == nil then return end -- Skip if no ESP objects
...
cleanupConnection = LocalPlayer.AncestryChanged:Connect(function()
    if not LocalPlayer:IsDescendantOf(game) then
        -- Player is leaving the game, perform cleanup
...
Tabs.EspTab:Slider({
    ...
    Step = 1,
    Value = {Min = 1, Max = 10000, Default = 1500},
    Callback = function(value)
