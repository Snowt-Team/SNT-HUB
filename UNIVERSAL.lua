...
local function getClosestPlayerToMouse()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
            ...
            if onScreen and (not AimbotConfig.LockInFOV or (mousePos - Vector2.new(screenPos.X, screenPos.Y)).Magnitude < AimbotConfig.FOVRadius) then
...
-- Setup UI callbacks
local function setupAimbotCallbacks()
    local function updatePlayerList()
    ...
    Tabs.AimbotTab:Toggle({
        ...
        Callback = function(state)
            ...
            if not state and Camera.CameraSubject ~= (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")) then
                Camera.CameraSubject = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") or Camera.CameraSubject
            end
        end
    })
